...
gdf = gdf[gdf["iso3"] != "ATA"].copy() # Remove Antarctica
...
def make_sym_quasilog_boundaries(lim_val, linthr):
    ...
    for e in range(emin, emax + 1):
        for m in (1, 2, 5):
            ...
            if linthr <= v <= lim_val:
                pos_edges.append(v)
    pos_edges = sorted(set(pos_edges + [lim_val]))  # ensure lim is included
...
extend_mode = "both" if (clipped_low and clipped_high) else ("min" if clipped_low else ("max" if clipped_high else "neither"))
