...
def load_admin_mixed(path: str, layer: str) -> gpd.GeoDataFrame:
    ...
    # Remove Antarctica robustly: drop polygons with min latitude <= -60
    bbox = out.bounds  # minx, miny, maxx, maxy in EPSG:4326
    out = out[bbox["miny"] > -60].copy()

    # Project to Robinson
    out = out.to_crs(CRS_PROJ)
    return out
