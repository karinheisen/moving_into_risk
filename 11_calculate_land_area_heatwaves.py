...
raster = rioxarray.open_rasterio("raster_netMgr_2000_2019_annual.tif", masked=True)

mean_migration = raster.isel(band=slice(0, 16)).mean(dim="band", skipna=True)

# Downscale from 5 arcmin -> 30 arcmin
...
def relative_diff_percent(mean_pos, mean_neg):
    """
    Relative difference in percent, using out-migration as reference:
        ((E_in - E_out) / |E_out|) * 100
    Returns np.nan if mean_neg is zero or not finite.
    """
...
# ---------------------- Core computation for one model ----------------------
def compute_stats(nc_hist, nc_cf, label):
    """
    Compute present, trend, counterfactual, and dropout-based
    robustness tests on the relative difference.
    ...
    print(f"  Present-day exposure (real):          + {pres_real_pos:.4f}, - {pres_real_neg:.4f}, rel diff = {pres_real_rel:.1f}%")
    print(f"  20th century trend (real):            + {trend_real_pos:.4f}, - {trend_real_neg:.4f}, rel diff = {trend_real_rel:.1f}%")
    print(f"  Present-day exposure (counterfactual): + {pres_cf_pos:.4f}, - {pres_cf_neg:.4f}, rel diff = {pres_cf_rel:.1f}%")
    print()

# ---------------------- Dropout resampling test ----------------------
    ...
    def bootstrap_relative(values):
        """
        Distribution of relative differences under dropout:
            ((E_in - E_out) / |E_out|) * 100
        """
        diffs = []
    ...
    print(f"  Dropout rel diff (present-day real): {b_pres_real_mean:.1f}% "
          f"[{b_pres_real_ci[0]:.1f}%, {b_pres_real_ci[1]:.1f}%], robust={b_pres_real_robust}")
    print(f"  Dropout rel diff (20th c. trend real): {b_trend_real_mean:.1f}% "
          f"[{b_trend_real_ci[0]:.1f}%, {b_trend_real_ci[1]:.1f}%], robust={b_trend_real_robust}")
    print(f"  Dropout rel diff (present-day CF): {b_pres_cf_mean:.1f}% "
...
results_20cr = compute_stats(
    nc_hist="hwmid-humidex_20crv3_historical_heatwavedarea_global_annual_landarea_1901_2016.nc",
    nc_cf="hwmid-humidex_20crv3_picontrol_heatwavedarea_global_annual_landarea_1901_2016.nc",
    label="20CRv3"
)

results_gswp = compute_stats(
    nc_hist="hwmid-humidex_gswp3-w5e5_historical_heatwavedarea_global_annual_landarea_1901_2016.nc",
    nc_cf="hwmid-humidex_gswp3-w5e5_picontrol_heatwavedarea_global_annual_landarea_1901_2016.nc",
    label="GSWP3-W5E5"
)
